...
- Every public package lives under `packages/*`, is independently versioned,
  and must remain installable without a checkout of VelarScript Core.
...
- `@velarscript/editor-kit` owns only editor connection metadata and helpers.
...
- Run the narrow package checks first, then `npm run validate`. Validation must
  ...
  for release.
- Preserve unrelated work. Commit, push, tag, npm publication, and deprecation
