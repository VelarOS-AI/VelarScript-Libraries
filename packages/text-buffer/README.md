...
count lines by line feed (`\n`) alone, because they are built on
`Text.lineStarts`. A lone carriage return does not start a line: a CR-only
document is one line to this buffer. The VelarScript compiler's own line
numbering treats `\r\n`, a lone `\r`, and `\n` all as terminators, so
diagnostics reported by the toolchain do not necessarily agree with the line
numbers reported here. Map positions through one model or the other, never
both.
